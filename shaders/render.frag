...
void main()
{
    ...
    {
        const vec3 viewDir = normalize(centralViewDir+samplingDir*((sampleNum+0.5)/superSampleCount-0.5));
#else
    const vec3 viewDir=calcViewDir();
    {
        ...
        const bool viewRayIntersectsGround = lookingIntoAtmosphere && sin(opticalHorizonElevation(altitude)) > cosViewZenithAngle;
